...
YT_SHORT_REGEX = re.compile(r"^(?:https?://)?youtu\.be/([\w-]{11})")
...
def fetch_video_metadata(video_id):
    if video_id in _video_cache:
        cached_time, cached_data = _video_cache[video_id]
    ...
    try:
        ...
        if not response.get("items"):
            print(
                f"Advertencia: No se encontró el video {video_id} en YouTube.",
                file=sys.stderr,
            )
            return None
        ...
        _video_cache[video_id] = (time.time(), result)
        return result
        ...
        print(
            f"Advertencia: Error al consultar YouTube API: {e}",
            file=sys.stderr,
        )
        return None
...
def main():
    ...
    parser.add_argument(
        "--url", type=str, required=True, help="URL del video de YouTube"
    )
    parser.add_argument(
        "--user-id", type=str, required=True, help="UUID del usuario"
    )
    ...
    if not video_id:
        print(
            "Error: No se pudo extraer el video_id de la URL.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        if result.data:
            ...
            print(
                "Error: No se recibió respuesta de la base de datos.",
                file=sys.stderr,
            )
            sys.exit(1)
